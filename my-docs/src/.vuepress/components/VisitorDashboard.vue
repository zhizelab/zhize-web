<template>
  ...
  <div v-else-if="!canViewDashboard" class="auth-required-card">
    <h3>该页面需要登录后访问</h3>
    <p>{{ authStatusText }}</p>
    <div class="auth-required-actions">
      <button type="button" class="primary-btn" @click="goHomePage">返回首页登录</button>
      <button type="button" class="secondary-btn" @click="retryAuth">重新校验</button>
    </div>
  </div>
  <div v-else class="visitor-dashboard-final">
    <!-- 顶部 Header -->
...
const authStatusText = ref('正在校验登录状态...');
...
const runAuthCheck = async (): Promise<boolean> => {
  authStatusText.value = '正在校验登录状态...';
  try {
    ...
    authStatusText.value = '当前未登录或登录已过期，请先在左上角登录后再访问。';
  } catch (error) {
    authStatusText.value = error instanceof Error ? error.message : '登录校验失败，请稍后重试。';
  }
...
onMounted(async () => {
  ...
  // 必须在 canViewDashboard 变为 true 之后，DOM 元素才存在
  await bootstrapDashboard();
});
...
.primary-btn {
  background: linear-gradient(120deg, #dceeff 0%, #a5d4ff 100%);
  color: #18456f;
}
.secondary-btn {
  background: #eaf1f9;
